    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-blue-100 py-16">
      ...
      >
        <h1 className="text-5xl font-extrabold text-gray-900 sm:text-6xl">
          About <span className="text-indigo-600">Kickstart</span>
        </h1>
        ...
        >
          {[
            ...
            <motion.div
              ...
              whileTap={{ scale: 0.97 }}
              className="p-8 rounded-xl bg-white/90 shadow-lg border border-gray-200 backdrop-blur-lg hover:border-indigo-500 transition-all duration-300 ease-in-out transform hover:scale-[1.05] hover:shadow-2xl"
            >
              <div className="flex items-center mb-4">
                <motion.div whileHover={{ rotate: 10, scale: 1.2 }}>
                  <item.icon className="h-10 w-10 text-indigo-600" />
                </motion.div>
                <h2 className="text-2xl font-bold text-gray-900 ml-3">{item.title}</h2>
              </div>
              <p className="text-gray-600">{item.text}</p>
              <button
                onClick={() => navigate(item.link)}
                className="mt-4 px-5 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-all duration-300"
              >
                Learn More
              </button>
            </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto px-4">
          {[
            ...
            <motion.div
              ...
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.97 }}
              className="p-6 rounded-lg bg-white/90 shadow-md border border-gray-200 backdrop-blur-lg hover:border-indigo-500 transition-all duration-300 ease-in-out transform hover:scale-[1.05] hover:shadow-2xl"
            >
      ...
      <motion.div
        ...
        whileInView={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.8, ease: "easeOut" }}
        className="mt-16 max-w-5xl mx-auto bg-white/90 p-8 rounded-lg shadow-md border border-gray-200 backdrop-blur-lg"
      >
